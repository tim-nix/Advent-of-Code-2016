...
if __name__ == '__main__':
   ...
   # Start at the '5' button at index (1, 1).
   current = [ 1, 1 ]
   ...
   for y in range(len(instructions)):
      for x in range(len(instructions[y])):
         ...
         # Correct the current position if step
         # results in being off of the keypad.
         if current[0] < 0:
            current[0] = 0
         if current[1] < 0:
            current[1] = 0
         if current[0] == len(keypad):
            current[0] = len(keypad) - 1
         if current[1] == len(keypad[0]):
            current[1] = len(keypad) - 1
