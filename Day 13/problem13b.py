...
#   an open space.
# - If the number of bits that are 1 is odd, it's
#   a wall.
#
# How many locations (distinct x,y coordinates,
# including your starting location) can you reach
# in at most 50 steps?
...
if __name__ == '__main__':
   ...
   # Will store the fartherest number of steps to
   # be taken.
   final_distance = 50
   ...
   while not found:
      ...
      # Is the current distance greater than the
      # farthest number of steps to be taken? If
      # so, then done.
      if distance == final_distance + 1:
         found = True
         # All visited locations are stored in the
         # 'visited' set, so return its length.
         final_count = len(visited)
   ...
   # Display the number of locations that can be
   # visited in the provided number of steps.
   print("Number of locations visited = " + str(final_count))
